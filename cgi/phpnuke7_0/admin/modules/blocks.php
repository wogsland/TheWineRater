...
function BlocksAdmin() {
	...
	while(list($bid, $bkey, $title, $url, $bposition, $weight, $active, $blanguage, $blockfile, $view) = sql_fetch_row($result, $dbi)) {
	    $weight1 = $weight - 1;
	    $weight3 = $weight + 1;
	    $res = sql_query("select bid from ".$prefix."_blocks where weight='$weight1' AND bposition='$bposition'", $dbi);
	    list ($bid1) = sql_fetch_array($res, $dbi);
	    $con1 = "$bid1";
	    $res2 = sql_query("select bid from ".$prefix."_blocks where weight='$weight3' AND bposition='$bposition'", $dbi);
...
function block_show($bid) {
    ...
    OpenTable2();
    $bid = intval($bid);
    $result = sql_query("select bid, bkey, title, content, url, bposition, blockfile from ".$prefix."_blocks where bid='$bid'", $dbi);
    list($bid, $bkey, $title, $content, $url, $bposition, $blockfile) = sql_fetch_row($result, $dbi);
    if ($bkey == main) {
        mainblock();
    } elseif ($bkey == admin) {
        adminblock();
...
function fixweight() {
    ...
    $rightpos = "r";
    $centerpos = "c";
    $result = sql_query("select bid from ".$prefix."_blocks where bposition='$leftpos' order by weight ASC", $dbi);
    $weight = 0;
    ...
    $result = sql_query("select bid from ".$prefix."_blocks where bposition='$rightpos' order by weight ASC", $dbi);
    $weight = 0;
    while(list($bid) = sql_fetch_row($result, $dbi)) {
	$weight++;
    $bid = intval($bid);
    ...
    $result = sql_query("select bid from ".$prefix."_blocks where bposition='$centerpos' order by weight ASC", $dbi);
    $weight = 0;
    while(list($bid) = sql_fetch_row($result, $dbi)) {
	$weight++;
    $bid = intval($bid);
...
function BlocksAdd($title, $content, $url, $bposition, $active, $refresh, $headline, $blanguage, $blockfile, $view, $expire, $action) {
    global $prefix, $dbi;
    if ($headline != 0) {
    ...
    $result = sql_query("SELECT weight FROM ".$prefix."_blocks WHERE bposition='$bposition' ORDER BY weight DESC", $dbi);
	...
	sql_query("insert into ".$prefix."_blocks values (NULL, '$bkey', '$title', '$content', '$url', '$bposition', '$weight', '$active', '$refresh', '$btime', '$blanguage', '$blockfile', '$view', '$expire', '$action')", $dbi);
	Header("Location: admin.php?op=BlocksAdmin");
    }
}
...
function BlocksEdit($bid) {
    ...
    echo "<br>";
    $bid = intval($bid);
    $result = sql_query("select bkey, title, content, url, bposition, weight, active, refresh, blanguage, blockfile, view, expire, action from ".$prefix."_blocks where bid='$bid'", $dbi);
    list($bkey, $title, $content, $url, $bposition, $weight, $active, $refresh, $blanguage, $blockfile, $view, $expire, $action) = sql_fetch_row($result, $dbi);
    if ($url != "") {
	$type = _RSSCONTENT;
    } elseif ($blockfile != "") {
	$type = _BLOCKFILE;
...
function SortWeight($bposition) {
    ...
    $numbers = 1;
    $number_two = 1;
    $result = sql_query("SELECT bid,weight FROM ".$prefix."_blocks WHERE bposition='$bposition' ORDER BY weight", $dbi);
    ...
    $result_two = sql_query("SELECT bid,weight FROM ".$prefix."_blocks WHERE bposition='$position_two' ORDER BY weight", $dbi);
...
function BlocksEditSave($bid, $bkey, $title, $content, $url, $oldposition, $bposition, $active, $refresh, $weight, $blanguage, $blockfile, $view, $expire, $action) {
    global $prefix, $dbi;
    if ($url != "") {
	$bkey = "";
	$btime = time();
	...
	if ($oldposition != $bposition) {
	    $result = sql_query("select bid from ".$prefix."_blocks where weight>='$weight' AND bposition='$bposition'", $dbi);
	    $fweight = $weight;
	    $oweight = $weight;
	    ...
	    $result = sql_query("select bid from ".$prefix."_blocks where weight>'$oweight' AND bposition='$oldposition'", $dbi);
	    while (list($obid) = sql_fetch_row($result, $dbi)) {
		sql_query("update ".$prefix."_blocks set weight='$oweight' where bid='$obid'", $dbi);
		$oweight++;
	    }
	    $result = sql_query("select weight from ".$prefix."_blocks where bposition='$bposition' order by weight DESC limit 0,1", $dbi);
	    list($lastw) = sql_fetch_row($result, $dbi);
	    if ($lastw <= $fweight) {
		$lastw++;
		sql_query("update ".$prefix."_blocks set title='$title', content='$content', bposition='$bposition', weight='$lastw', active='$active', refresh='$refresh', blanguage='$blanguage', blockfile='$blockfile', view='$view' where bid='$bid'", $dbi);
	    } else {
		sql_query("update ".$prefix."_blocks set title='$title', content='$content', bposition='$bposition', weight='$fweight', active='$active', refresh='$refresh', blanguage='$blanguage', blockfile='$blockfile', view='$view' where bid='$bid'", $dbi);
	    }
	} else {
	    $result = sql_query("update ".$prefix."_blocks set bkey='$bkey', title='$title', content='$content', url='$url', bposition='$bposition', weight='$weight', active='$active', refresh='$refresh', blanguage='$blanguage', blockfile='$blockfile', view='$view' where bid='$bid'", $dbi);
	}
	Header("Location: admin.php?op=BlocksAdmin");
    } else {
	$title = stripslashes(FixQuotes($title));
	...
	if ($oldposition != $bposition) {
	    ...
	    $result = sql_query("update ".$prefix."_blocks set bkey='$bkey', title='$title', content='$content', url='$url', bposition='$bposition', weight='$weight', active='$active', refresh='$refresh', blanguage='$blanguage', blockfile='$blockfile', view='$view', expire='$expire', action='$action' where bid='$bid'", $dbi);
	}
	Header("Location: admin.php?op=BlocksAdmin");
    }
}
...
function BlocksDelete($bid, $ok=0) {
    ...
    $bid = intval($bid);
    if ($ok) {
	$result = sql_query("select bposition, weight from ".$prefix."_blocks where bid='$bid'", $dbi);
...
switch($op) {
    ...
    case "BlocksAdd":
    BlocksAdd($title, $content, $url, $bposition, $active, $refresh, $headline, $blanguage, $blockfile, $view, $expire, $action);
    break;
    ...
    case "BlocksEditSave":
    BlocksEditSave($bid, $bkey, $title, $content, $url, $oldposition, $bposition, $active, $refresh, $weight, $blanguage, $blockfile, $view, $expire, $action);
    break;
